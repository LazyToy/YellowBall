...
import type { ShopScheduleViewRow } from "@/lib/super-admin-data"
...
type NewStringBookingPageClientProps = {
  scheduleRows: ShopScheduleViewRow[]
}

export function NewStringBookingPageClient({ scheduleRows }: NewStringBookingPageClientProps) {
  ...
  const handleSubmit = () => {
    if (!date || !slot || !racketId || !stringId) return
    ...
    if (date === toKstDateKey() && slot <= currentTime) {
      setFeedbackError("현재 시간 이후의 예약 시간을 선택해 주세요.")
      return
    }
  ...
  return (
    <div className="flex-1 flex flex-col">
      <ActionFeedbackDialog
        ...
        description="확인을 누르면 예약 목록으로 이동합니다."
        onConfirm={() => router.push("/booking")}
      />
      <ActionFeedbackDialog
        open={feedbackError !== null}
        title="예약 시간을 확인해주세요"
        description={feedbackError ?? undefined}
        tone="danger"
        onConfirm={() => setFeedbackError(null)}
      />
