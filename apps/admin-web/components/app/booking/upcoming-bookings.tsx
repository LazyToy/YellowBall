...
const steps = ["접수", "승인", "작업중", "완료"] as const
...
export function UpcomingBookings({ storeName }: { storeName?: string }) {
  const displayStoreName = storeName || 'YellowBall'

  // storeName을 목업 데이터에 반영
  const localBookings = bookings.map((b) => ({ ...b, location: displayStoreName }))

  return (
    <section className="px-5 pt-4 space-y-3">
      {localBookings.map((b) => {
        const Icon = b.type === "string" ? Wrench : Sparkles
        const isPending = b.status === "접수"
        return (
          <article
            key={b.id}
